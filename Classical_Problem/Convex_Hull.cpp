...
vector<vector<int>> Convex_Hull(vector<vector<int>> nums)
{
	int point_nums = nums.size();
	vector<vector<int>> order;
	//保存新的顺序
	vector<vector<int>> order_n;
	int minum = INT_MAX;
	int row_start = 0;	//凸包开始点的x坐标
	int col_start = 0;	//凸包开始点的x坐标
	int pos = 0;
	double k1 = 0;
	double k2 = 0;
	...
	order.push_back(vector<int>{row_start, col_start});
...
int main()
{
	vector<vector<int>> num = { {-1, -2},{ -4, 2 },{ -2, 4 },{ -1, 1 },{ 0, 2 },{ 1, 0 },{ 4, -1 },{ 5, 0 } };
